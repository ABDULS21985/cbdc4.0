...
contract CBDC is ERC20, Ownable, Pausable {
    ...
    function depositFor(address to, uint256 amount, bytes memory signature) public {
        // We need to recover the 'from' address (signer) from the signature
        // The message signed should include (to, amount, contractAddress) - Nonce removed to match doc interface
        ...
        // emit OfflineDeposit(signer, to, amount, nonce); // Nonce not available
        emit OfflineDeposit(signer, to, amount, 0); 
    }
